...
if ! python "${LIB_PATH}/python_scripts/annotation_tagger_linker_metagenomics.py" \
    --input "$INPUT_FILE" \
    --output "$OUTPUT_DIR"; then
    echo "Error: annotation_tagger_linker_metagenomics.py failed for file: $INPUT_FILE" >&2
    exit 1
fi
