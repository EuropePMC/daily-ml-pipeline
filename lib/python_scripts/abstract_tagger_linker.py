...
def format_output_annotations(all_linked_annotations, ext_id, source, provider, types):
    """
...
def generate_output_basename(input_basename):
    ...
    Handles both date-based and 'total'-based filenames.

    Args:
        input_basename (str): The input filename (e.g., 'patch-28-10-2024-0.abstract.gz' or 'patch-total-123.abstract.gz').

    Returns:
        str: The output filename (e.g., 'patch-2024_10_28-0' or 'patch-total-123').

    Raises:
        ValueError: If the input basename does not match any expected format.
    """
    try:
        # Pattern for date-based filenames: patch-dd-mm-yyyy-123.abstract.gz
        date_pattern = r'^patch-(\d{1,2}-\d{1,2}-\d{4})-(\d+)\.abstract\.gz$'
        # Pattern for 'total'-based filenames: patch-total-123.abstract.gz
        total_pattern = r'^patch-(total)-(\d+)\.abstract\.gz$'

        match = re.match(date_pattern, input_basename)
        if match:
            date_str, index_str = match.groups()
            formatted_date = datetime.strptime(date_str, "%d-%m-%Y").strftime("%Y_%m_%d")
            return f"patch-{formatted_date}-{index_str}"

        match = re.match(total_pattern, input_basename)
        if match:
            keyword, index_str = match.groups()
            return f"patch-{keyword}-{index_str}"

        raise ValueError(f"Input basename does not match expected formats: {input_basename}")
